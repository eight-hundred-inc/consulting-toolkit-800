...
| `transcriptPath` | 親が**整形済みで保存したトランスクリプトのプレーンテキスト**の絶対パス（例: `/tmp/circleback-minutes-<id>/transcript_<slug>.txt`）。話者付き・タイムスタンプ除去済みで、そのまま生成スキルに渡せる |
...
`meetingDetailsPath`（会議詳細 JSON）と `transcriptPath`（整形済みトランスクリプト `.txt`）を Read で読み込む。
...
`transcriptPath`（`transcript_<slug>.txt`）は親が整形済みのプレーンテキスト。形式は「話者名: の行＋発言」のブロックを空行区切りで並べたもの（タイムスタンプ・JSON 構造は除去済み）。**ワーカーはこれを再整形せず、そのまま使う**。
...
`transcriptPath` の `.txt` は親側で決定論的に整形済み（話者付き・タイムスタンプ除去・連続する同一話者の連結済み）。**ワーカーは時系列整形・話者統合・タイムスタンプ変換を行わない**（旧仕様の整形処理は親 Python に移管済み）。読み込んだテキストをそのまま生成スキルへの入力に使う。

**`.txt` が空または空白のみの場合**（トランスクリプト未取得）: `outputPath` に以下の最小ファイルを書き込み、`status: incomplete` で終了する。
...
## トランスクリプト
[整形済みトランスクリプト（transcriptPath の .txt 内容をそのまま）]
...
- **文字起こし**: 整形済みトランスクリプト（transcriptPath の .txt 内容をそのまま）
