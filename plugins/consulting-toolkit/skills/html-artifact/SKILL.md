...
| 5 | [references/diagram-components.md](references/diagram-components.md) | **図解の統合リファレンス**：固定 8 図解（概念フロー・2x2・ピラミッド・ファネル・サイクル・ベン図・組織図・レイヤー）＋**レイアウトパターン**（スイムレーン/マトリクス/カード/ステップ/タイムライン。image-generator-guide 取り込み）＋**作り込み図版**（`.fig-NN` per-figure）。図解が必要なときに参照 |
...
5. **コンポーネント選択**
   ...
   - **レイアウトパターン／作り込み図版（Slide Deck format・既定）**：Slide Deck で、メッセージが構造的（スイムレーン／マトリクス／時系列・ガント／多層／対比／関係・アーキ）で、かつ上記 8 図解・25 コンポーネントで表現しきれない場合は、**既定で** `references/diagram-components.md` 後半の **レイアウトパターン（5 種）または作り込み図版（`.fig-NN`）**を構成する。配色は `--fig-accent` 由来 + `--good`/`--warn` に限定（テーマ追従・**多色禁止は維持**）。8 図解・既存コンポーネントで表現できる構造はそちらを優先。**opt-out**：「シンプルでよい／作り込み不要」の指示があれば 8 図解・コンポーネントにフォールバックする
...
- `references/diagram-components.md` — 図解の統合リファレンス（固定 8 図解＋レイアウトパターン 5 種＋作り込み図版 `.fig-NN`）
