...
| 4 | [references/components.md](references/components.md) | 25 種のコンポーネント仕様（基本 21 種＋拡張 4 種。HTMLスニペット付き）。**末尾に付録「Markdown → HTML マッピング」**（旧 markdown-html-mapping.md を統合） |
...
9.5 **作り込み図版の委譲生成（Slide Deck format・リッチ判定該当図のみ）**
    ...
    リッチ判定に該当する作り込み図版は、**1 図 1 体の専用ワーカー `slide-figure-creator`（agent）に委譲**するのを既定とする。デッキ全体を組みながら片手間に図を作ると 1 図あたりの注意が希釈され品質が落ちるため（図単位の設計・クロップ検証・反復を担保する）。

    - (a) **委譲対象の確定**：step 5 でリッチ判定に該当し opt-out でない図版を列挙し、`fig-NN` を**親が事前採番**する（SVG marker id `figNN-ah` の一意性も担保）
    - (b) **ハーネス生成（1 回）**：`assets/template-slides.html` の `:root` 全トークン（確定テーマで `--accent` 群を上書き）＋`.fig-wrap`/`.fig-canvas`（`--fa-*` ランプ含む）＋Google Fonts link＋Content スライド枠（title-bar＋message＋図版スロット）を含む単一スライド HTML を `/tmp/slide-figs-<id>/harness.html` に書く。**ハーネスの body は `padding:0`** にする（padding があると 1280px の `.slide` が viewport からはみ出し、`overflow:hidden` が図版の両端を切る「偽クリップ」が出る。検証済みの落とし穴）
    ...
    - (e) **回収と統合**：各ワーカーの fragmentPath の内容を**丸ごと**対応スライドの図版スロットに貼る（フラグメントは `.fig-NN` スコープの `<style>`＋`.fig-wrap` の自己完結形式）。`effectiveHeight` が図版領域（≈440px）を大きく超える図は step 11 で重点確認する
    ...
    **委譲をスキップしてよい条件**（親が直組み）：図版が 1〜2 枚で構造が単純／ユーザーが「シンプルでよい／軽くで」と指示（既存 opt-out と整合）／サブエージェントを起動できない環境。スキップ時も 1 図 1 サイクル（図単位で組んで都度検証）は守る。
...
Slide Deck format は **5 テーマ共通の同一シェル**を使う。テーマ切替は `:root` の `--accent` / `--accent-soft` / `--accent-bg` の 3 変数のみで行う（シェルの仕様は `references/slide-deck.md`）。
...
| **Mono** | **投資家ピッチ、コンサル提案書、外部向けハイステークス資料** |
...
- `references/components.md` — 25 種のコンポーネント仕様（基本 21 + 拡張 4）＋付録「Markdown → HTML マッピング」
