...
**図版のキャプション冒頭に置くインライン番号（ステップ番号 `01`／`02` 等）は対象外**：番号だけを `JetBrains Mono` や弱色に変えず、同じキャプション内のテキストと同一のフォント・色・サイズにする。`JetBrains Mono` はキャプション本文と別行・別要素の数字（doc-id・章番号・ページ番号・KPI 強調数値等）にのみ使う（詳細は diagram-components.md「フォント」）。
...
### 同一ボックス・同一行内でのフォントサイズ変更を禁止

**同一のボックスの同一の行内では、フォントサイズを変えない**（インライン `<span>` で一部だけ拡大・縮小しない）。数値と単位を並べる場合（`kpi-value`＋`.unit`、`bc-amount`＋`.u`、`hn-value`＋`.hn-unit` 等）も対象で、単位・補助テキストは本体と**同一のフォントサイズ**にする。強弱・控えめさは色（`var(--ink-mute)` 等）や `font-weight`、余白で表現し、サイズ差では表現しない。行を分けて別要素（別のブロック・別行）にする場合はこの制約の対象外（例：Hero Number の `.hn-value` 本体行と `.hn-label`/`.hn-note` の説明行は別行なのでサイズが異なってよい）。

### サイズの目安

| 要素 | サイズ | weight | letter-spacing |
|------|--------|--------|----------------|
| cover-title (h1) | 48px | 700 | 0.01em |
| cover-sub | 22px | 400 | normal |
| section-title (h2) | 30px | 600 | 0.01em |
| sub-head (h3) | 20px | 600 | normal |
| minor-head (h4) | 16px | 700 | normal |
| 本文 p | 16px | 400 | 0.01em |
| メタテキスト | 13〜14px | 400 | 0.05〜0.2em |
...
**これは下限であって調整の余地ではない**（禁じ手⑧「行間ピッタリ」）。`line-height:1` / `1.2` を本文・リストに使わない。段落・項目の間にも余白を取る（段落間 14px 相当、`li{margin-bottom}`）。内容が器に入り切らないときに行間を詰めて詰め込まず、情報量を減らすかスライドを分ける。逆にブロック間ギャップを広げて余りを埋めるのも不可（`_shared/slide-body-principles.md` 原則 7）——**詰めるのも伸ばすのも禁じ手**で、換算表の値を保つ。

**対象は本文・リスト・表セル・見出しの「読ませる行」**。折り返さない単行の表示要素（Hero Number の `.hn-value` のような巨大数値、図版内の 1〜2 語ラベル）は、行間を詰めても段落がくっつく問題が起きないため対象外（既存の `line-height:1` / `1.25` はこの例外）。
...
- **禁止：辺の一部だけをアクセント色にする**（`border-left` / `border-top` 等、面の 1 辺だけを `var(--accent)` にする「アクセント縦線・横線」）。minor-head・insight・lede・qa-card・flow-note・dq-cell.accent・summary 等でかつて使っていたが廃止した。ミニデッキ全体でアクセントが多用され「1 色だけ」の原則が崩れるため。強調したい場合は罫線ではなく **面全体のティント**（`background:var(--accent-bg)`）か、**罫線を全周とも `var(--rule)` の中立色にする**（面としての区切りは保ちつつ着色しない）。意味のある警告色（`--warn`）を辺の一部に使うのは例外として許可する（例：`.flow-note.warn{border-left:3px solid var(--warn)}`）
...
> 本表と別軸で、**紙書き研修「15 の禁じ手」**（正本：`${CLAUDE_PLUGIN_ROOT}/skills/_shared/kinjite-15.md`）が全出力形式に適用される。両者は整合しているが、競合したときは禁じ手が優先する。下表の末尾 7 行は禁じ手由来の項目。
...
| **幅調整のための `<br>`・`&nbsp;`・全角スペース**（禁じ手②） | 枠幅を変えると右端が段々になり、修正のたびに打ち直しになる。折り返しは要素幅と `line-height` に任せる。`<br>` は意味として 2 行に分けたい行のみ（区切り記号の `&nbsp;·&nbsp;` のような非改行スペースは対象外） |
| **`<p>` / `<div>` の羅列で箇条書きに見せる**（禁じ手③） | リスト構造が失われ、項目の追加・並べ替え・インデント変更が効かなくなる。箇条書きは必ず `<ul>/<ol>` ＋ `<li>`（マーカー字形の置き方は pptx-safe.md §1） |
| **「入り切らないから」の文字サイズ縮小／並列要素間のサイズ不統一**（禁じ手⑥） | 同じ役割の要素（カード本文・リスト項目・表セル）は**デッキ全体で同一の `font-size`**。1 枚だけ・1 カードだけ縮めない。入らないなら情報量を減らすかスライドを分ける（`_shared/slide-body-principles.md` 原則 5） |
| **英数字・カタカナの全半角混在**（禁じ手⑥） | **英数字は半角・カタカナは全角**に統一する（`ﾓﾃﾞﾙ`／`モデル`、`FY10`／`ＦＹ１０` の混在は不可）。単位・記号（`%` `→` `・`）の表記もデッキ内で揃える |
| **行間ゼロ詰め（`line-height:1`〜`1.2` の本文・項目間余白なし）**（禁じ手⑧） | 段落がくっついて読めない。上記「line-height」の下限を守り、項目間は `li{margin-bottom}` で余白を取る。縦が足りなくても行間を詰めて詰め込まない（逆に間延びで埋めるのも不可＝原則 7） |
| **スライド本体のアニメーション（`animation` / `@keyframes` / 出現エフェクト）**（禁じ手⑭） | 設定・管理に工数を取られ、印刷と pptx 変換では意味をなさない。`transition` は印刷時 `display:none` のプレゼンチャーム（カウンタ・サムネイル・操作ヒント・リンクのホバー）に限る。段階的な説明はスライドを分ける |
| **3D・立体・3 軸のチャート**（禁じ手⑮） | 軸と比較対象が増えてメッセージが伝わらない。比較軸が 3 つあるならチャートを分ける。奥行き・斜め投影・3D 円グラフ・立体積層棒はすべて不可（「回転・斜め配置」の禁止と同根） |
...
- [ ] 絵文字を使っていない
- [ ] 禁じ手②：幅調整のための `<br>` / `&nbsp;` / 全角スペースが無い
- [ ] 禁じ手③：箇条書きが `<ul>/<ol>` ＋ `<li>`（`<p>`/`<div>` の羅列で偽装していない）
- [ ] 禁じ手⑥：並列要素の `font-size` が揃っている／英数字は半角・カタカナは全角
- [ ] 禁じ手⑧：本文・リストの `line-height` が下限以上で、項目間・段落間に余白がある
- [ ] 禁じ手⑭：スライド本体に `animation` / `@keyframes` が無い（`transition` はチャームのみ）
- [ ] 禁じ手⑮：3D・立体・3 軸のチャートが無い
- [ ] 残りの禁じ手（①④⑤⑦⑨⑩⑪⑫⑬）は `_shared/kinjite-15.md` のチェックリスト A で確認した
