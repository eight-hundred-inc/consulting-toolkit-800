...
本ファイルは**図版1点の内部構造**（作図文法・固定8図解・図版内レイアウト）のリファレンスである。**スライド1枚全体のコンテンツエリア構造**は ${CLAUDE_PLUGIN_ROOT}/skills/slide-pattern-creator/library/（SLIDE-PATTERN）が正本であり、本ファイルでは扱わない。レイアウトパターン（スイムレーン/マトリクス/カード/ステップ/タイムライン）の記述は、図版内部のレイアウトに限って適用する。
...
> **8 図解で足りないとき**：以下に (A)「作り込み図版」と (B)「レイアウトパターン（image-generator-guide 取り込み）」を統合した。選び方は次節「図解の 3 層」のリッチ判定に従う。
...
3. **作り込み図版**（後述 §作り込み図版）。上記で形にならない、そのスライド固有の構造を per-figure CSS で 1 枚だけ作り込む。組み方は §作り込み図版の「exemplar 方式」に従う。

## リッチ判定（Slide Deck では 3 層ルールに優先する）

Slide Deck format では、メッセージが次の**いずれか**に当てはまる場合、固定 8 図解で形式上表現できても
**作り込み図版を既定**とする。固定 8 図解の均一な箱では
属性・関係・次元の情報が落ち、単調な「箱の列」になるため。

- **ノードが属性を 2 つ以上持つ** — 名称＋役割＋正本/状態 等（例：ツール構成図の各ツール）
- **辺・受け渡しに意味のあるラベルが付く**（例：「受注時に案件を自動生成」）
- **次元が 2 つ以上ある** — レーン × 時系列、2 軸 N×M、層 × 中身
- **1 要素だけを強調するヒーロー対比** — 推奨案・中核ツール・目的ブロック

固定 8 図解を使うのは**単一関係・単一次元の概念図**（短いフロー・サイクル・ベン図・シンプルなピラミッド等）に限る。

**リッチ判定はレイヤー選択の設計ガイドであって、委譲するか否かのゲートではない**：Slide Deck format では固定 8 図解・作り込み図版を問わず全構造化図版を `slide-figure-creator` に委譲する（SKILL.md step 9.5）。リッチ判定は、ワーカーが固定 8 図解（layer 1）で組むか作り込み図版（layer 3）で組むかを判断するための基準として使う。ユーザーが「シンプルでよい／作り込み不要」と指示した場合は、委譲はスキップせず、ワーカーが固定 8 図解・既存コンポーネント寄りの軽いレイヤーを選ぶ。

**組み方**：§作り込み図版の「exemplar 方式」と「型に当てはめて情報を落とさない」に従う。
...
## レイアウト原則（image-generator-guide 踏襲・構造型で選ぶ）

作り込み図版の作図方法論は image-generator-guide（image-creator サブエージェント）と同じ流儀を採る。原則は以下に折り込み済みで、本ファイルだけで完結する。

**レイアウト方式はメッセージの構造型で選ぶ**：

| 構造型 | レイアウト方式 |
|---|---|
| 並置・対比／ステップフロー／マトリクス N×M／タイムライン・ガント／層スタック | **flow（grid / flex）既定** |
| ハブ&スポーク／アーキ図（辺ラベルが主役）／スイムレーン（レーン跨ぎの曲線）／空間配置が意味を持つ図 | **absolute＋SVG が第一候補**（作図文法の座標計算規律＋図単位検証とセットで使う） |

- **flow（grid / flex）**：隣接・整列・等間隔はレイアウトエンジンに任せ、座標計算を発生させない（「辺がノードに届かない」「バッジが浮く」事故を構造的に防ぐ）。要素間隔は `gap`、ラベル列・矢印列・バッジ列は **px 固定幅**、メインコンテンツ領域だけ `flex:1`。矢印・コネクタは固定幅の flex アイテムとして要素間に挟む（インライン SVG または疑似要素）
- **flow の矢印にもラベルを付けてよい**：矢印カラム（固定幅 flex アイテム）に受け渡しの小ラベル／ピル（10〜11px）を載せる。「flow にしたら辺ラベルが消えた」という退行を起こさない
- **absolute＋SVG**：放射状の辺・レーン跨ぎ曲線・辺上ラベル・空間配置など、flow では表現できない／表現力が大きく落ちる構造に使う。高品質なアーキ図・スイムレーンの exemplar はこの方式で組まれている。座標は必ず計算で決める（作図文法 step 5）
- **改行制御**：日本語ラベルは `white-space:nowrap` を基本とし、改行が必要な箇所は `<br>` で明示する。自然な区切りで改行し、単語の途中で切らない

## exemplar 方式（ゼロから発明しない）

作り込み図版は、テンプレート集に内容を流し込むのではなく、**出来の良い既存図版（exemplar）を参照して組む**。

1. **構造の出発点**：本ファイル §レイアウトパターン（スイムレーン / マトリクス / カード / ステップ / タイムラインの CSS スケルトン）から、メッセージの構造型に合うものを選ぶ
2. **作り込みの見本**：過去案件で承認済みの図版 HTML（過去デッキの `.fig-NN`、image-creator / image-generator-guide の出力、改修元のオリジナル図版）があれば開いて参照する。**改修・移植で元図版があるときは元の構造に忠実に**組む
3. 見本から移植するのは**文法**（トークン化・濃淡ランプ・scoped CSS・SVG 辺の idiom）。絶対配置の見本を移植する場合、**ジオメトリ（scale 値・座標・寸法）はコピーせず本節の式で再計算**する。レイアウト方式は §レイアウト原則の構造型に従う：absolute 第一候補群（ハブ&スポーク・アーキ・スイムレーン）は**元図版の absolute 構造に忠実に**組み、flow 既定群のみ flow へ寄せてよい
4. **1 図 1 サイクル**：図単位で組んだら都度レンダリングして確認する。デッキ全体の最後にまとめて検証しない

### 型に当てはめて情報を落とさない

図の構造は**メッセージの構造**に従わせる。レイアウトパターン・見本はあくまで出発点で、合わなければ型の側を変形する。
典型的な失敗：時系列に流れるステップ列を「フェーズ列のセルにステップを積む」形に丸めると、
担当の情報は残るが**順序が消えてプロセスとして読めなくなる**。
判定基準：完成した図を見て**元のメッセージの読み順・順序・関係が 3 秒で追えるか**。
追えなければ当てはめをやめ、メッセージの構造（元図版があるならその構造）から組み直す。

## fig-canvas — 全図版共通のラッパー

すべての作り込み図版を `.fig-wrap > .fig-canvas.fig-NN` に載せる。`--fig-accent` と濃淡ランプは `.fig-canvas` にスコープされているため、**flow 図版でもこのラッパーを外さない**。マークアップは `assets/template-slides.html` にスキャフォルド済み：
```html
<div class="fig-wrap"><div class="fig-canvas fig-01"><!-- 図版 --></div></div>
```

**flow 図版（既定）**：スライド本文領域の幅（padding 内 = **1152px**）にそのまま組む。`transform:scale` も高さ補正も不要。フォントは表示サイズで直接指定する（ラベル 14px・説明 11.5px 目安。スライド内の他コンポーネントと整合）。
```css
.fig-01{ width:1152px; }
```

**絶対配置図版（例外）**：大きいネイティブサイズで設計し、`transform: scale` で本文領域に縮小する。
```
...
- 過去図版の値（`1488px / 0.78` 等）はそのデッキ固有のジオメトリなので**コピーしない**。
```css
.fig-01{ width:1400px; transform:scale(0.823); }
```

## 絶対配置キャンバスの高さ補正（重要・落とし穴。flow 図版には不要）
...
- 配色は `--fig-accent`（既定 `--accent`、ブランド色はデッキ単位で上書き）由来の `color-mix` 濃淡ランプ＋`--good`/`--warn` のみ。ハードコード色は使わない。濃淡ランプは `assets/template-slides.html` の `.fig-canvas` に**同梱済み**（テンプレ以外から始めたデッキでは以下を一度だけ定義する）：
```css
/* ===== 濃淡ランプ（.fig-canvas にスコープ。template-slides.html に同梱済み） ===== */
.fig-canvas{
  --fa-04: color-mix(in srgb, var(--fig-accent) 4%,  #fff);   /* 最淡面（行ヘッダー・空セル） */
  --fa-08: color-mix(in srgb, var(--fig-accent) 8%,  #fff);   /* 淡面（カード淡背景） */
...
- 仕上げ確認：`--fig-accent` が**彩度の高い色（青等）と無彩色（Mono / Charcoal）の両方**で破綻しないか見る
...
4. **レイアウト方式を選ぶ**：§レイアウト原則の構造型表で判定する。並置・ステップ・マトリクス・タイムライン・層は flow（grid/flex）、ハブ&スポーク・アーキ図（辺ラベル主役）・スイムレーン（レーン跨ぎ曲線）・空間配置は absolute＋SVG が第一候補。
5. **ジオメトリは計算で決める（絶対配置のみ・目分量禁止）**：先にノードの `left/top/width/height` を確定し、辺の発着点（ノード矩形の縁）・辺ラベル（辺の中点付近）・注釈バッジ（アンカー要素の縁から 8〜12px）の座標を**ノード座標から逆算**して置く。等間隔のカラム配置はカラム幅 `w = (キャンバス幅 − ラベル列幅) / 要素数` から各中心座標を先に算出する。注釈・バッジを対象から離して置く場合は**リーダー線で接続**する（接続なしで空中に浮かせない）。
6. **図版領域を使い切る（最重要）**：flow 図版は表示サイズ（1152px 幅・ラベル 14px 目安）で直接組み、領域に対して小さければ**ノードの `min-height`・padding・`gap` を増やして自然高を ≈440〜460px まで伸ばし**、残差はスライド側の flex 縦中央寄せで吸収する（小さい図を上に張り付けて下半分を空白にしない）。スライド側の縦中央寄せはシェルの `fig-slide` クラスが担保する（slide-deck.md「図版スライドは fig-slide で縦領域を使い切る」）。絶対配置図版はネイティブ 1300〜1500px・文字 16〜24px で設計し、ネイティブ高を高さ予算（≒ 440 / scale）に近づける → トークン/ランプで塗る → 検証。**図が縦の上 1/3 に縮こまる現象は品質劣化の主因**なので最優先で潰す。

## 検証（3 経路・必須）

`.fig-canvas` の scale は `.slide` のビューポート scale、さらにサムネイルの `scale(0.125)` と三重に入れ子になる。**①ライブ ②印刷（PDF・`@page 1280×720`）③サムネイル** の 3 経路で確認する。`@media print` が `.slide` の transform だけ解除し、子孫 `.fig-canvas` の scale を消さないこと。

合格判定は「はみ出し・クリップ・重なりがない」だけでは**不十分**。スクリーンショットで**接続と整列**まで確認する：

- [ ] 辺・矢印の**両端がノード矩形の縁に接している**（途中で切れたスタブ・どこにも繋がらない矢印がない）
- [ ] 注釈・バッジ・辺ラベルが**アンカー要素に隣接**している（離す場合はリーダー線で接続されている）
- [ ] 対応関係のある要素（左右 2 列の行と中央の矢印 等）の**位置が揃っている**
- [ ] 図が**図版領域の高さを使い切っている**（上部 1/3 に縮こまって下半分が空白、になっていない）
- [ ] 日本語ラベルが**意図しない位置で改行されていない**（必要箇所は `<br>` で明示されている）
- [ ] フォントサイズが読みやすく、要素間の**余白バランス**が取れている
- [ ] はみ出し・クリップ・重なりがない

はみ出しの機械測定は `scrollWidth`/`clientWidth` の比較（レイアウト箱基準）で行う。`getBoundingClientRect` は `.slide` のビューポート scale の影響を受けるため誤判定する。

### 委譲時の役割分担（slide-figure-creator）

- **ワーカー（図単体）**：本節のチェックリスト全項目を、`screenshot.py --selector .fig-canvas` のクロップで図ごとに完了する（合格まで反復）
- **親（統合）**：組み上げ後のデッキで、スライド全景サンプリング・`scrollWidth` はみ出し・図版とテキストの重なり・プレゼンチャーム/カウンタ/サムネイル・`fig-NN`/SVG marker id の衝突ゼロを確認する（図単体の再検証はしない）
...
| 出力 | PNG（screenshot.py・body自動クロップ） | **デッキ内インライン**。スライドは fig-canvas に載せる（flow は 1152px 直組み・絶対配置のみ scale フィット）、Vertical Document は幅可変で可 |
...
<svg class="fig-edges" viewBox="0 0 W H">
  <defs><marker id="figNN-ah" markerWidth="9" markerHeight="9" refX="6.5" refY="3.2" orient="auto">
    <path d="M0,0 L7,3.2 L0,6.4 Z" fill="currentColor"/></marker></defs>
  <path d="M..." fill="none" stroke="var(--fig-accent)" stroke-width="2.4" marker-end="url(#figNN-ah)"/>
</svg>
```

- **重なり順（検証済みの落とし穴）**：辺レイヤー `z-index: 5` ＜ ノード `10` ＜ 辺上ラベル `12` の順に重ねる。ラベルをノードより下に置くと見切れる
- marker の `id` は `figNN-ah` のように**図ごとに一意**にする（同一デッキ内の SVG `id` 衝突防止）
- 辺はノードの**矩形の縁**から発着させる（座標はノードの `left/top/width` から逆算）。辺上ラベルは辺の中点付近に絶対配置で重ねる
...
- **時系列プロセスでは 1 ステップ＝1 カラムを崩さない**。ステップをフェーズ列のセルに積み上げると、担当の情報は残るが**順序が消えてプロセスとして読めなくなる**。ステップ 12 超はカードを縮めず**フェーズでスライドを分割**する。
- レーン跨ぎの受け渡し＝太い accent 実線＋ラベルピル、同一レーン内の工程移動＝細い破線で描き分け、**凡例で両者の区別を明示**する。
