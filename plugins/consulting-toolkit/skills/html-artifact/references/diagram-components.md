...
6. **図版領域を使い切る**：flow 図版は表示サイズ（1152px 幅・ラベル 14px 目安）で直接組み、領域に対して小さければ**ノードの `min-height`・padding・`gap` を増やして自然高を伸ばし**、残差はスライド側の flex 縦中央寄せで吸収する（小さい図を上に張り付けて下半分を空白にしない）。絶対配置図版はネイティブ 1300〜1500px・文字 16〜24px で設計し、ネイティブ高を高さ予算（≒ 440 / scale）に近づける → トークン/ランプで塗る → 検証。
