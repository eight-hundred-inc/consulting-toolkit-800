...
Step 3（提案用）・Step 11（報告用）が出力するスライド構成 MD は、必要に応じて HTML スライドや PPTX へ実体化する（11 ステップの外側・任意工程）。スライド生成は **内容構成・レイアウトパターン・スライドマスター・実装** の 4 層に分離されており、レイアウトパターンの割付は実装側（html-artifact のレイアウト割付）が行う。構成 MD に `パターン指定: SLIDE-PATTERN-{name}` が書かれていればどの実装経路でも優先される（詳細はリポジトリ README「スライド化フロー」を参照）。

```
スライド構成 MD ─────────────────┬─▶ html-artifact（16:9 HTML デッキ・作り込み図版）＝主経路
...
| ステップ | 成果物 | 格納先 |
|----------|--------|--------|
| 初期化, Step 1〜 | プロジェクトサマリ | `プロジェクトサマリ.md` |
