...
SubAgent（エージェント定義ファイル）を対話形式で作成するスキル。

## 入出力

- **入力**: ユーザーからの要件（目的・役割・トリガー条件）
- **出力**: SubAgentエージェント定義ファイル（`.md`）
...
もしSkillの方が適切な場合は、`skill-creator` スキルがインストールされていればそちらに処理を移譲する。インストールされていない場合は、ユーザーにSkill作成の基本手順（SKILL.mdのフロントマター構成、ディレクトリ配置）を案内する。
...
1. **保存場所**
   ...
   - ユーザーレベル: `~/.claude/agents/` - すべてのプロジェクトで使用
...
**保存先**:
- プロジェクト: `.claude/agents/[subagent-name].md`
- ユーザー: `~/.claude/agents/[subagent-name].md`
...
### SubAgentが見つからない
- ファイルが `.claude/agents/` または `~/.claude/agents/` にあるか確認
- ファイル拡張子が `.md` か確認
