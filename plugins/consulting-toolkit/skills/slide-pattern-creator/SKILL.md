...
> **マスターは配色だけでなくフレーム構造まで規定する場合がある**：html-artifact の大半のテーマ（Terracotta / Navy / Forest / Charcoal / Mono）は `--accent` 系の配色差分のみだが、クライアントブランド固有テーマ（例：`EightHundred`）は実 PPTX に合わせて Cover 全面背景・title-bar のアイブロウラベル・footer のコピーライト表記・ロゴマークまで規定する。詳細は html-artifact の `references/design-system.md`「Theme 6: EightHundred」を参照。パターン側（本スキル）が扱うのはあくまでタイトル行より下のコンテンツエリアのみで、この差はパターン定義に影響しない。
...
パターンは html-artifact（HTML）だけでなく pptx（ネイティブ PowerPoint）でも実装される。pptx では **1 図形の中のテキストは段落として縦に積まれる**ため、「1 行の中に横並びの要素がある」構造は再現できない。パターン定義の `content` は、この制約を踏まえて **行の内訳ではなく行の役割**で書く。

- ✗ 「番号バッジ（左）＋見出し（右）を横並び」 → pptx で上下 2 段に分解される
- ○ 「番号付きの見出し行」（番号を行の一部として扱う。バッジにするかは下流のルック判断）
- ✗ 「ゾーン全体に矢印オーバーレイを重ねる」 → 1 枚の画像として貼られるか線に分解されるかで揺れる
- ○ 「ゾーン間の遷移を示す矢印（1 対 1 で 1 本ずつ）」→ pptx のコネクタになり、図形を動かしても追従する

塗り付きチップ・丸番号バッジを**構造の必須要素にしない**（ルックの選択肢として下流に委ねる）。とくに **1 文字コードの塗りチップ（`G` / `K` 等のセマンティックコード）と、テキストで足りる円形の番号・記号バッジ（`1` `2` `3` `↑` 等）は構造にも見本にも書かない**——読み手に情報を足さず、pptx では本文と別図形に分解されて縦位置が合わないため（html-artifact の `references/design-system.md` 禁止パターン）。番号・記号・コードは**行のテキストの一部**として書く。詳細は html-artifact スキルの `references/pptx-safe.md`。

### 禁じ手 15 のうち、構造の段階で決まるもの

紙書き研修「15 の禁じ手」（正本：`${CLAUDE_PLUGIN_ROOT}/skills/_shared/kinjite-15.md`）のうち、**パターン定義＝構造の段階で決まってしまう**項目がある。ここで作り込んでおくと、下流（html-artifact / branded-pptx）がパターンに従うだけで禁じ手を踏まなくなる。逆にここで踏むと、そのパターンを使う全スライドに伝播する。

| # | 禁じ手 | パターン定義で守ること |
|---|---|---|
| ① | タイトル＆メッセージ直打ち | **タイトル行はパターンの対象外**（スライドマスターが定義する）。パターン側でタイトルの位置・大きさを規定しない。これは「分析対象外（スライドマスターで定義）」の既定どおり |
| ③ | 箇条書きの点直打ち | `content` に「リスト N 項目」と書いた箇所は、**下流でリスト構造（`<ul>/<li>`・pptx の箇条書き段落）として実装される**前提。`notes` に「段落の羅列ではなくリストとして組む」と書く。マーカーの字形・色は書かない（ルック） |
| ⑤ | モノ文字ボックス | 1 ゾーンの `content` に内容を全部押し込まない。**意味の塊ごとにゾーン／要素を分け**、`content` に要素の種類と数を書く。分けた並列要素の幅は揃える（下記⑪） |
| ⑪ | サイズバラバラ | 並列ゾーン・並列カードは**等分**を既定にし、非等分なら `width` に比率を明記する。**塗りも枠も持たないゾーンも同じ幅・同じ余白**にする（見えない不揃いも pptx では不揃いな図形になる）。`notes` に「並列要素は同数・同幅・下端揃え」と書く |
| ⑫ | 縦横配置バラバラ | ゾーンは矩形分割で定義し、`reading_order` を明示する。ゾーンの左端・上端が揃う分割にする（斜め・オフセット配置のパターンを作らない） |
| ⑬ | 線でマトリックス | マトリクス型のパターンは**セル（面）の集合**として定義する。「罫線で区切った領域」ではなく「N×M のセル」と書く。セル内に見出しと本文が入ることを `content` に明記する |
| ⑮ | 3 軸グラフ | 3 軸・3D のチャートを前提としたパターンを作らない。比較軸が 3 つある構造は、**軸を分けた複数チャートのパターン**として定義する |

②（改行＆スペース）・④（インデント）・⑥（フォント）・⑦（下線）・⑧（行間）・⑨（図形に文字ボックス）・⑩（コネクタ）・⑭（アニメーション）は**ルックまたは実装の段階の項目**なので、パターン定義（schema v2）には書かない。ただし **STEP 5 のスケルトン HTML は下流が採寸・参照する見本**なので、そちらでは全項目を守る（STEP 5 参照）。
...
**下流実装ヒント：** html-artifact: [使用コンポーネントの目安（例：State Grid、2カラム flow）]／pptx: [プレースホルダ配置・図形の目安（例：左に画像プレースホルダ、右にテキストボックス2つ）。**1 行に横並びの要素が要るゾーン、ゾーンを跨ぐ矢印がある場合はここに注意書きを添える**（例：番号は見出し行の一部として1つのテキストに入れる／矢印はゾーン間に1本ずつ独立して置く）]
...
- **PPTX 変換セーフな書き方で組む**（プレビューは下流が採寸・参照する見本のため、崩れる書き方を見本にしない）：
  - `::before` / `::after` で箇条書きマーカー・罫線・矢印を描かない（マーカーは行のテキストに `● ` として書く／罫線はその要素の `border-top` / `border-bottom`）
  - 1 行を「マーカー要素＋本文要素」に分けない。行を包む要素が本文のテキストノードを直接持つ（`<span>01</span>見出し`）
  - `transform: scale` / グラデーション背景 / `box-shadow` だけの境界を使わない（境界は `border` で引く）
  - 字下げは `padding-left`＋負の `text-indent` で作る（`margin-left` は箱ごとずれ、その行の罫線・背景が本文左端からずれる）。**行ごとのアンダーライン（`border-bottom`）は付けない**（pptx では行ごとに線シェイプ化し、折り返し差で揃わない）
  - **面（カード・パネル・帯・バー）は角丸を使わず直角にする**（`border-radius:0`）。とくに **2 色構成（塗りヘッダー帯＋本文）のカードで角丸は使わない**（pptx では上 2 角／下 2 角だけ丸める専用シェイプに分割され、継ぎ目に線が入る・重ねた 2 枚がずれる）。円形（`border-radius:50%`）とピル形のチップ／バッジのみ例外
  - 表を含むパターンでは罫線の色・太さ・実線/点線を全セルで統一する。セルごとに罫線を変えると、pptx では差分のある辺ごとに独立した図形が増える（強調はセルの `background-color` で表現する）
  - **同一オブジェクトの箇条書き（1 つの `<ul>`/`<ol>`）を `display:grid`（`grid-template-columns` 2 列以上）や `column-count` で多段組にしない**。常に単一列で縦積みにする（項目間の余白は `li{margin-bottom}`）。項目を意味のある単位で 2 列に「並べたい」場合は、1 つのリストを割るのではなく **独立した `<ul>` を 2 つ**横に並べる構造にする
  - **面（カード・パネル・ラベル帯）の辺の一部だけを強調色にする構造（`border-left` / `border-top` だけを太く／濃くする「アクセントバー」）で組まない**。スケルトンはグレースケールのみだが、この構造のまま html-artifact 側で実テーマを当てると `var(--accent)` が 1 辺だけに乗り、「アクセントは 1 色・意味の区別にのみ」の原則が崩れる。強調したい面は全周を同じ太さ・同じ色の罫線にするか、背景の濃淡だけで差をつける
  - **隣接する要素（ステップバッジ・フェーズ帯・カード等）に濃淡違いの単色を順に並べて進行・段階を表現しない**（見た目がグラデーションになるため禁止。例：`vertical-step-three-col` の旧版は STEP1〜5 のバッジを `#888→#444` の順に濃くしていたが、全ステップ同一濃度に統一した）。進行はステップ番号・連結線・矢印で示し、色は変えない
  - **塗り面だけで意味を伝える装飾アイコンを描かない**：1 文字コードの塗りチップ（`G` / `K` 等）と、テキストで足りる円形の番号・記号バッジ（`1` `2` `3` `↑` 等）は見本に書かない。番号は `<span>01</span>見出し` のように**行のテキスト**として置く（塗り・円形にしない）。図解のノード番号・タイムラインのマーカー・凡例ドット・チャートの要素のように、**位置や大小そのものが意味を持つ図形要素は対象外**
  - 詳細は html-artifact スキルの `references/pptx-safe.md`
- **禁じ手 15 を全項目守る**（正本：`${CLAUDE_PLUGIN_ROOT}/skills/_shared/kinjite-15.md`）。スケルトンは下流が採寸・参照する見本なので、**禁じ手を踏んだ書き方を見本にしない**。上記の変換セーフ規約でカバーされない項目は次のとおり：
  - **幅調整のための `<br>`・`&nbsp;`・全角スペースを使わない**（禁じ手②）。ダミーテキストの折り返しは要素幅に任せる。`<br>` は意味として 2 行に分けたい行だけに使う
  - **箇条書きは `<ul>/<li>` で書く**（禁じ手③）。`<div>` を並べて箇条書きに見せない（リスト構造が下流に伝わらない）
  - **並列要素の `font-size` を揃える**（禁じ手⑥）。ダミーテキストが入り切らないからといって 1 か所だけ小さくしない（入らないならダミーの文字数を減らす）。11px 未満にしない
  - **英数字は半角・カタカナは全角**で書く（禁じ手⑥）。ラベル・ダミーテキストの表記をファイル内で統一する
  - **`line-height` は 1.7 以上**、項目間・段落間に余白を取る（禁じ手⑧）。ゼロ詰めにしない
  - **並列ゾーン・並列カードは等分 flex/grid（`flex:1` / `1fr`）＋下端揃え**で組み、個別の `width`/`height` を px で微調整しない（禁じ手⑪）。**塗りも枠も持たないゾーンも同じ幅・同じパディング**にする——矩形は色に関係なく実測されるため、見えない不揃いが pptx の不揃いになる
  - **`position:absolute`＋px の微調整で並べない**（禁じ手⑫）。ゾーン分割は flex / grid で行い、左端・上端・ギャップを揃える
  - **マトリクス型はセル（面）で組む**（禁じ手⑬）。罫線を引いた領域に絶対配置テキストを載せる書き方をしない。`<table>` の `<td>` か grid のセル `<div>` を並べる
  - **`animation` / `@keyframes` / `transition` を一切使わない**（禁じ手⑭。スケルトンに JavaScript を使わないのと同じ理由）
  - **3D・立体・3 軸のチャート表現を見本にしない**（禁じ手⑮）
...
- ラベルは必ずすべてのエリアに表示する
- ダミーテキストは日本語で記述する
- 書き終えたら `${CLAUDE_PLUGIN_ROOT}/skills/_shared/kinjite-15.md` の**チェックリスト A（15 項目）**を通す。①（タイトル行）はマスターの領域なので「パターン側で規定していない」ことを確認する
