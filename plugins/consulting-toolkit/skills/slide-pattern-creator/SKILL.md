...
このスキルはスライド生成4層アーキテクチャの **2層＝レイアウトパターン** を担う。1枚のスライドのコンテンツエリア構造の唯一の正本であり、上流の `slide-structure-designer`（構成MD）が構図欄でパターンを指名し、下流の実装スキル（`html-artifact` / `image-creator` / `branded-pptx`）がそれに従う。下流は **パターン名＋構成MDの図版指示** で構造に従うのが既定で、スケルトンHTMLは列比・固定幅などの **比率採寸に迷うときの任意参照** とする（優先順位＝構成MD構図 ＞ 図版指示 ＞ HTML寸法採寸）。
...
このファイルはスライドのコンテンツエリア（タイトル行より下の領域）のレイアウトパターン定義書です。スライドマスター（branded-pptx のマスター定義 / html-artifact のテーマ）と組み合わせてAIツールに渡すことで、このパターンのスライドを生成できます。タイトルエリア・ページ番号・フッター・装飾はスライドマスターで定義されるため、このファイルには含みません。
...
**対応する図解指名：** [`slide-structure-designer` の構図欄で使う語彙（例：左右2カラム / 概念フロー / 2x2マトリクス / ピラミッド / ファネル 等）。上流がこの語彙でパターンを指名できるようにする。該当なしなら「―」]
**下流実装ヒント：** html-artifact: [使用コンポーネント・テーマの目安（例：State Grid、2カラム flow）]／pptx: [プレースホルダ配置・図形の目安（例：左に画像プレースホルダ、右にテキストボックス2つ）]

## Structure（構造）

[コンテンツエリアの構造を1〜3文で説明する。タイトル行より下のエリアがどう分割されているかを記述する。タイトル・ページ番号・フッター・装飾は含めない。]
...
> 幾何の細かな数値（列幅 px・余白）はここに固定せず、STEP 5 のスケルトンHTMLを正本とする（後述の「正本ルール」）。YAML は意味構造——エリアの数・種類・関係・強調——を担い、非自明な比率だけ `split: 40/60` のようなヒントを残す。
...
> **正本ルール（幾何は HTML が正本）**：スケルトンHTMLは、そのパターンの**空間的な幾何（列比率・カラム幅・余白・行数）の正本**である。STEP 4 の定義MD（Structure YAML）は**意味構造（エリアの数・種類・関係・強調ルール）の正本**とし、幾何は「均等」「左右2分割」等の言語表現か、非自明な比率のときだけ最小ヒント（例：`split: 40/60`）にとどめる。**同じ幾何数値を YAML と HTML の両方にハードコードしない**——二重管理は drift（両者の数値が食い違う不整合）の原因になる。ピクセル単位の列幅・グリッド定義が必要な下流（branded-pptx 等）は、YAML の意味構造で足りない分だけ HTML を採寸して読む。
>
> **下流での優先順位（重要）**：段組・カード数・グリッド形（2×2 か 1×4 か）・ステップ数といった**意味構造は YAML が正本**であり、その上位に**構成MDの構図指定**が立つ。スケルトンHTMLの具体形がこれらの意味構造を上書きしてはならない（HTMLは寸法の採寸源であって、段組やカード数の決定者ではない）。**優先順位＝ 構成MDの構図指定 ＞ YAML意味構造 ＞ HTML寸法採寸**。この順を守らないと、HTMLの一具体例（例：1×4）が構成側の指定（例：2×2）を上書きして構図逸脱を招く。したがってスケルトンHTMLは、そのパターンの**標準的な意味構造をそのまま体現する一例**として作り（YAMLと段数・カード数を一致させる）、恣意的な簡略化・別解を混ぜない。
...
今回生成したパターンの内容（概要・適したシーン・対応する図解指名）をもとに、`{OUTPUT_ROOT}/SLIDE-PATTERN-INDEX.md` の**最も近いカテゴリの末尾**に追記する。
...
作成するファイルの形式：
    ...
    新しいパターンを追加した際は、このファイルの該当カテゴリに追記してください。
