...
AIが判断しきれない意思決定はステップの前後に人間レビューポイントを入れる。AIステップは完了後にユーザー承認のレビューゲートを経て次へ進む。`quality-reviewer` の起動は `review_level: full` を付与したステップに限り、`light` のステップはユーザー確認のみで進む（判定ロジックは project-manager/SKILL.md「レビューゲート」節に従う）。review_level は、成果物がクライアント提出物または後続ステップの入力になる場合のみ full とする。
...
| `quality-reviewer` | 成果物の品質レビュー | `review_level: full` のステップ、または提出前最終検査のときだけ起動（light のステップでは起動しない） |
| `image-creator` | 図解・画像生成の委譲実行 | 成果物のビジュアル化が必要で、コンテキスト分離したい場合。内部で `image-generator-guide` スキルを読み込む |
...
**連携スキルの選定基準**:
- ステップの目的・出力が、上記「利用可能なスキル・SubAgent」のいずれかの用途・出力に合致するかで判断する
- 合致するスキルがある場合、そのスキルを連携スキルとして指定し、実行手順にはスキルの読み込みと入力の受け渡しを書く
- 完全一致するスキルがない場合は「なし」とし、AIが直接実行できる具体的な手順を書く
- SubAgent（`desk-researcher`）は、大量の中間出力が発生しコンテキスト分離が必要な場合に選択する

---

## workflow.html 出力フォーマット

カスタムワークフローはプロジェクトルートに `workflow.html` として保存する。定義済みワークフロースキルの SKILL.md と references/phases.md を1ファイルに統合した構造。
...
定義済みワークフローをベースに修正する場合、変更内容を `workflow.html` に記録する。
...
## 変更点
- Step 4（インタビューガイド作成）: 省略
- Step 6（インタビュー対象者選定）: 省略
- (インタビュー実施): 省略
- Step 7（インタビュー議事メモ作成）: 省略
- Step 8（インタビューまとめ）: 省略
- Step 9（デスクリサーチ）: Phase 1の主要ステップに昇格
- Step 5A（追加）: ワークショップ設計 [AI] → 新規ステップ
```

変更点以外のステップは元のワークフロースキルの定義をそのまま使う。`workflow.html` には変更・追加したステップの詳細のみを記載する。
