...
ToolSearch のキーワード検索で 3 ツールを解決する。`+<名前>` は「その語を名前に含むツール」を要求する部分一致で、名前空間プレフィックスが付いていてもマッチする。3 ツールぶんを **1 メッセージにまとめて並列発行** する（`+` は 1 語 = 1 ツール要件のため 3 本に分ける）。
...
議事録 MD の生成とユーザー確認が完了した後の最終ステップとして、Notion のミーティング DB に正式議事録を反映する。**作業環境の CLAUDE.md（ルート等）に Notion のミーティング DB（DB_ミーティング）が定義されている場合のみ実施する。定義がなければこのステップはスキップ**する（DB ID をスキルにハードコードしない。環境ごとの CLAUDE.md の定義を参照する）。

背景ルール（議事録二層運用）:

- Circleback 自動生成の Notion ページ＝全会議のベースライン記録
- 本スキルで生成した正式議事録の**正はローカル MD**（Step 7 で決めた出力先）
- 同期方向は常に**ローカル → Notion の一方通行**。議事録を修正する場合もローカル MD を直してから再同期する（Notion 側だけを直さない）

反映手順（**親エージェントが実行する。ワーカーには任せない**。ワーカーはファイル生成のみの責務を維持）:

1. 各会議について、Notion のミーティング DB を会議名・日付で検索し、Circleback 自動生成ページを特定する（`notion-search` / `notion-query-data-sources` 等の Notion MCP ツールを使用。未ロードなら ToolSearch で解決する）
2. **置換前の確認（必須）**: 既存ページ本文の置換は破壊的操作のため、実行前に「対象ページ URL と会議名の対応表」をユーザーに提示して確認を取る。複数会議の一括処理では対応表をまとめて 1 回だけ確認する
3. 特定できたページの本文を、生成した正式議事録の内容で**置換**する。本文の冒頭または末尾にローカル MD のフルパスを記載する
4. 該当ページが見つからない会議は、ミーティング DB に**新規ページを作成**する（Meeting Name・Date を設定し、本文に正式議事録＋ローカル MD のフルパスを記載）
5. 反映結果（置換 / 新規作成 / スキップ）を会議ごとに 1 行で報告する
