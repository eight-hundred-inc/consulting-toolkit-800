...
|---|---|---|---|---|
| **Exa** (`…__web_search_exa`) | セマンティック（embedding） | URL + 要約 + フルコンテンツ | 技術文献、研究、英語ソース、類似事例の発見 | 日本語の最新ニュース、SNS、ローカルメディア |
| **Exa fetch** (`…__web_fetch_exa`) | 単一URL取得 | クリーンな本文（広告・ナビ除去） | 取得済みURLの本文化 | 動的レンダリングサイト |
...
> Exa のツール名は末尾（`web_search_exa` / `web_fetch_exa`）は共通だが、サーバー名プレフィックスはコネクタ登録ごとに異なる（例: `mcp__claude_ai_Exa__…` や `mcp__<uuid>__…`）。エージェント側は `tools:` でハードコードせず親から継承する設計のため、環境のコネクタ名を問わず利用できる。Exa が接続されていない環境では WebSearch / WebFetch を主に使う。
