...
エイトハンドレッド（800）社向けに構成した、コンサルティングプロジェクトの立ち上げから最終報告書の作成まで、ワークフロー全体をAIがサポートするスキル・コマンド・エージェント群。Claude Code のプラグインとして動作する。

調査プロジェクトでは、提案書の論点を軸にインタビューガイド・報告書骨子まで一貫して設計する。各スキルに品質基準が組み込まれており、AIタスクと人間タスクを明確に分離した協調型ワークフローで運用する。

## 全体像

project-manager がオーケストレーターとしてプロジェクトの状態を管理し、各ステップで実行スキルを呼び出す。重い処理（リサーチ・作図・品質レビュー）はサブエージェントに委譲し、一部のスキルは外部の MCP コネクタやローカルツールと連携する。
...
| [slide-figure-creator](plugins/consulting-toolkit/agents/slide-figure-creator.md) | html-artifact の HTML スライドデッキで、構造化図版（固定 8 図解～作り込み図版）を 1 図 1 エージェントで生成する専門ワーカー。設計→レンダ→クロップ検証→修正を反復し、デッキに埋め込む HTML フラグメントを返す | html-artifact スキルから並列起動（Step 9.5・1 図ごと） |
...
| 層 | 担当 | 責務 |
|---|---|---|
| 1 内容構成 | `slide-structure-designer` | 1 スライド 1 メッセージの構成 MD（タイトル/メッセージ/ボディ）。パターン割当は行わない（実装側のレイアウト割付に委ねる） |
...
flowchart LR
    ...
    MD ==>|"主経路"| HA["html-artifact<br/>16:9 HTML デッキ・作り込み図版"]
    MD -->|"PPTX 納品時"| PPTX["ブランド pptx（外部スキル）<br/>ネイティブ・編集可能 PPTX"]
    MD -.->|"限定用途"| IC["image-creator<br/>HTML→PNG 画像先行 → PPTX"]

    HA & PPTX & IC -.-> LIB[("② レイアウトパターン<br/>slide-pattern-creator / library<br/>全経路が参照する構造の正本<br/>136 パターン")]
```
...
- パターンの割付は html-artifact が Slide Deck 生成時にレイアウト割付（step 5。構造翻訳→割付→3 規範）で行う。構成 MD に `パターン指定:` が書かれていればそれを優先する（後方互換）
- ①から②③のどれに渡してもよい
