...
description: プロジェクトのワークフローを管理するPM（プロジェクトマネージャー）。「プロジェクトを開始」「新規プロジェクト」「ワークフロー開始」で初期化、`/pm-status`「進捗確認」「現在の状況」で状態レポート、「次のタスク」「続きを進めて」でワークフロー継続、「○○が完了」「○○しました」で人間タスク完了処理、「Agent Teamで〜」で並行処理を実行する。
---
...
5. **同期チェック**: `workflow.md` の論点サマリ・仮説ステータス・アプローチが、成果物（提案書等）の現在の内容と整合しているか確認する。乖離があれば `workflow.md` を更新する
6. 次のアクションを提案
...
3. 現在のステップの担当を確認:
   ...
   **AIタスクの場合**:
   ...
   - **自動同期ルールに従い `workflow.md` の動的セクションを更新**（論点・仮説・アプローチに変化があった場合）
   - **レビューゲートへ進む**（後述）
...
4. 成果物リンクを更新
5. 履歴に追記
6. **自動同期ルールに従い `workflow.md` の動的セクションを更新**（論点・仮説・アプローチに変化があった場合）
7. 次のステップを案内
...
3. `workflow.md`を更新
4. 報告内容に論点・仮説・アプローチの変化が含まれる場合（例: クライアント方針変更の伝達）、**自動同期ルールに従い `workflow.md` の動的セクションを更新**
...
プロジェクトの状態は `CLAUDE.md` と `workflow.md` の2ファイルで管理する。`CLAUDE.md` は静的（初期化時に一度だけ作成）、`workflow.md` は動的（ユーザーの明示指示なしに自動で更新）。

### 2ファイルの役割分担

| ファイル | 役割 | 自動ロード | 更新 |
|---------|------|-----------|------|
| `CLAUDE.md` | プロジェクトの静的基本情報（概要・作業上の注意・ファイル配置） | される | 初期化時のみ |
| `workflow.md` | ワークフロー進捗 + 動的文脈（論点・仮説・アプローチ・意思決定・チェックリスト・履歴） | されない（PMが読む） | 自動更新 |

- 基本情報（クライアント名・納期等）は `CLAUDE.md` にのみ持つ
- 論点・仮説・アプローチのサマリは `workflow.md` に持つ。詳細・根拠は `Output/提案書.md`（正本）を参照
...
以下の要素に変化が生じた場合、ユーザーの指示を待たず `workflow.md` を更新する。`CLAUDE.md` は更新しない。

| 変化の対象 | workflow.md の更新箇所 |
|-----------|----------------------------|
| 論点の追加・削除・変更 | 論点サマリ |
| 仮説の追加・状態変化 | 仮説ステータス |
| アプローチ・調査方針の変更 | アプローチ |
...
- レビュー承認でステップが完了した → `workflow.md` は必ず更新

変化がない場合は動的セクションを更新しない（不要な差分を生まない）。ステップ状態が変わるたびに進捗関連セクションは必ず更新する。
